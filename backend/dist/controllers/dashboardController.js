...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (appCache.has(CacheNameStrings.ADMIN_STATS)) {
        stats = JSON.parse(appCache.get(CacheNameStrings.ADMIN_STATS));
    }
    else {
        ...
        const latestTransactionsPromise = Order.find({})
            ...
            .limit(4);
        const [currentMonthsProducts, currentMonthsUsers, currentMonthsOrders, lastMonthsProducts, lastMonthsUsers, lastMonthsOrders, productsCount, usersCount, allOrders, lastSixMonthsOrders, categories, femaleUserCount, latestTransactions,] = await Promise.all([
        ...
        const currentMonthRevenue = currentMonthsOrders.reduce((total, order) => total + (order.total || 0), 0);
        const lastMonthRevenue = lastMonthsOrders.reduce((total, order) => total + (order.total || 0), 0);
        const changePercent = {
            revenue: calculatePercentage(currentMonthRevenue, lastMonthRevenue),
            product: calculatePercentage(currentMonthsProducts.length, lastMonthsProducts.length),
            user: calculatePercentage(currentMonthsUsers.length, lastMonthsUsers.length),
            order: calculatePercentage(currentMonthsOrders.length, lastMonthsOrders.length),
        };
        ...
        const categoryNameAndCount = await getCategoriesWithCount({ categories, productsCount });
        const userRatio = {
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (appCache.has(CacheNameStrings.PIE_CHARTS)) {
        pieCharts = JSON.parse(appCache.get(CacheNameStrings.PIE_CHARTS));
    }
    else {
        ...
        const [processingOrders, shippedOrders, deliveredOrders, categories, productsCount, productsOutOfStock, allOrders, allUsers, adminUsers, customerUsers,] = await Promise.all([
...
export const getBarCharts = TryCatch(async (req, res, next) => {
    ...
    if (appCache.has(CacheNameStrings.BAR_CHARTS)) {
        barCharts = JSON.parse(appCache.get(CacheNameStrings.BAR_CHARTS));
    }
    else {
        const today = new Date();
...
export const getLineCharts = TryCatch(async (req, res, next) => {
    ...
    if (appCache.has(CacheNameStrings.LINE_CHARTS)) {
        barCharts = JSON.parse(appCache.get(CacheNameStrings.LINE_CHARTS));
    }
    else {
        const today = new Date();
        const twelveMonthsAgo = new Date();
