...
export const myOrders = TryCatch(async (req, res, next) => {
  ...
  if (appCache.has(myOrderCacheName)) {
    ...
    orders = JSON.parse(appCache.get(myOrderCacheName) as string);
  } else {
...
export const allOrders = TryCatch(async (req, res, next) => {
  ...
  if (appCache.has(CacheNameStrings.ALL_ORDERS)) {
    console.log("in the if block");
    orders = JSON.parse(appCache.get(CacheNameStrings.ALL_ORDERS) as string);
  } else {
    console.log("in the else block");
...
export const getSingleOrder = TryCatch(async (req, res, next) => {
  const { id } = req.params;
  if (!id) return next(new ErrorHandler("Order id is Invalid", 400));
  ...
  if (appCache.has(myOrderCacheName)) {
    order = JSON.parse(appCache.get(myOrderCacheName) as string);
  } else {
